...
locals {
  ...
  # Reading, writing and removing the state object itself.
  state_actions = [
    "cos:GetObject",
    "cos:PutObject",
    "cos:HeadObject",
  ...
  # The COS backend does NOT lock with a .tflock object the way S3 once did.
  # It takes a lock in the Tencent *tag* service, on the tag key
  # `tencentcloud-terraform-lock`. Without these three actions the backend
  # cannot acquire or release a lock and every apply fails on lock timeout.
...
resource "tencentcloud_cam_policy" "state_access" {
  ...
  document = jsonencode({
    ...
    statement = concat([
      ...
      ], var.state_lock_enabled ? [
      {
        # Tag actions cannot be scoped to a single tag key in CAM, so this one
        # statement is account-wide. It is narrow in verbs rather than in
        # resources -- the alternative is no state locking at all.
        effect   = "allow"
        action   = local.lock_actions
        resource = ["*"]
      },
    ] : [])
  })
