...
# in envs/*/backend.hcl at review time.
#
# These files are inputs to `terraform init`, not to `terraform plan`, so
# writing them from Terraform does not create a cycle: envs/* read them long
...
resource "local_file" "env_backend" {
  ...
  filename        = "${path.module}/../envs/${each.key}/backend.hcl"
